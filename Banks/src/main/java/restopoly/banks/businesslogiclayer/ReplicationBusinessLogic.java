package restopoly.banks.businesslogiclayer;
...
public class ReplicationBusinessLogic {
    ...
    public void lockBankMutex(String gameid, String host, int port) throws InterruptedException {
        if (null == masterUri) {
            ...
            banksLogic.getBank(gameid).muhtex.lock();
    ...
    public void unlockBankMutex(String gameid, String host, int port) {
        if (null == masterUri) {
            ...
            banksLogic.getBank(gameid).muhtex.unlock();
    ...
    public boolean isUnlockedBankMutex(String gameid) {
        if (null == masterUri) {
            Lock mutex = banksLogic.getBank(gameid).muhtex;
            if (mutex.tryLock()) {
                mutex.unlock();
                return true;
            }
